...
A runnable networking and AI email-writing workspace with guided Finance and Academic outreach, persistent background jobs, open registration, a read-only administrator console, and Next.js, React, TypeScript, Tiptap, FastAPI, SQLAlchemy, Alembic, and PostgreSQL.
...
Includes live SerpAPI discovery, canonical LinkedIn profile URLs, Apify professional profiles and optional work-email lookup, multi-provider writing, independent saved personas/contacts/drafts, asynchronous suggestions, reusable email templates, connected sequence planning, and `.eml` export. Finance and Academic use the same provider chain; Academic changes the profile, search labels, recommendation context and writing scenarios rather than introducing a separate faculty directory. Gmail supports separate mailbox authorization, reviewed single-email sending and scheduling, contact-filtered Inbox, original-thread replies and manual follow-up reminders. See [Gmail setup and verification](docs/gmail-integration.md). Live Gmail use requires server configuration and mailbox-owner consent. Sequences remain planning only; automatic sequence execution is not implemented.
...
The seed command creates 1 fictional persona, 3 saved fictional contacts, and 2 drafts; running it again will not create new ones. Finance Mock search includes 16 fictional professionals and Academic Mock search includes 12 fictional mentors; any Mock email addresses use the reserved `.example` domain. There are no real people, and no fabricated public-profile evidence links.
...
**Academic guided workflow**: The **Domains → Academic** menu follows the same five-step structure for a student or researcher contacting a potential supervisor. Create an Academic persona, search by academic title, institution, location and research keywords, inspect the prepared public professional profile, choose a contact, then write and review a PhD, research internship or RA inquiry. Discovery uses the same **SerpAPI Google search → canonical LinkedIn URL → Apify profile → explicit work-email lookup** chain as Finance. Search and pagination never fetch email automatically. The MVP does not depend on a faculty-directory crawler or a global mentor database; those remain later data-quality improvements. See the [Academic domain MVP plan](docs/academic-domain-plan.md).
...
| --- | --- | --- |
| Finance & Academic People Search / Email Enrichment | `PEOPLE_MODE=mock` | `PEOPLE_MODE=live` + `SERPAPI_API_KEY` (search), `APIFY_API_KEY` (profiles/work email), `APOLLO_API_KEY` (optional Apollo email) |
...
- People Search: SerpAPI `GET /search.json`, `engine=google`, query `site:linkedin.com/in/` plus the active domain's fields. Academic requests use `/api/academic/search/jobs` and retain the shared wire fields `title`, `company`, `location`, `keywords`, and `sector`; the UI labels these Academic title, Institution / university, Location, Research keywords, and Research area. Finance uses the corresponding professional labels. Both return up to 10 Google hits per page, filter out non-person URLs and duplicates, and save each person's canonical LinkedIn URL. Google total is an estimate, not a count of verified people or a complete faculty roster; pagination follows Google's next-page signal.
...
## Shared SerpAPI, LinkedIn, Apify, and Email Design

The implemented sequence for both Finance and Academic is **SerpAPI Google discovery → canonical LinkedIn URL → automatically prepared Apify professional profile for the requested page → displayed Contact**. **Apollo/Apify work-email retrieval** and **AI drafting** remain separate actions. A page is displayed after each profile has either completed or recorded an explicit unavailable/error outcome; no next-page profiles or emails are automatically requested. The canonical LinkedIn URL is the identity key, and returning to the same search reuses the Contact without discarding a previously enriched email. See [profile preparation and live-provider verification](docs/people-provider-verification.md).
